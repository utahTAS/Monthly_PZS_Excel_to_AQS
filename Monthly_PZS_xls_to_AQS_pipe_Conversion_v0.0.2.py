...
for filename in os.listdir(directory):     #loop through files in user's dir
    if filename.lower().endswith('.xls') or filename.lower().endswith('.xlsx'):
        xls_df=pd.read_excel((directory + '\\' + filename), n_rows=4, usecols="A:O", converters=converters)
        ...
        if gap_count>0:
            ...
            print ("No PZS gaps greater than 2 weeks in {}!!!".format(filename))
                    
    #        print(PZS_test)
subset=output_df.columns[:10]
output_df=output_df.drop_duplicates(subset =subset, keep ='last')
