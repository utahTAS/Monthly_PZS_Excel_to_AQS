...
#sites_loc=r'U:/PLAN/BCUBRICH/Python/Parameter Reader/'\
#r'PARAMETERS.xls'
#
#sites_df=pd.read_excel(sites_loc, converters={'SITE NAME':str,'State Code':str,
#                                          'County Code':str, 'Site Code':str,
#                                          'Paramter':str, 'Analyt':str, 
#                                          'Method':str, 'Unit':str}).drop_duplicates(['SITE NAME','State Code','County Code','Site Code']) # load data

#sites_df.to_csv(r'U:/PLAN/BCUBRICH/Python/Monthly_PZS_xls_to_AQS_pipe_Conversion/temp.csv')
#sites=sites_df['SITE NAME'].unique
...
1,Smithfield,SM,49,005,00007,44201,(O3),1,087,007
...
df_test.columns=df_test.loc['index',:]
df_test=df_test.drop('index', axis='index')
...
for filename in os.listdir(directory):     #loop through files in user's dir
    if filename.lower().endswith('.xls') or filename.lower().endswith('.xlsx'):
        ...
        site_name=sites_df[(sites_df['County Code']==xls_df.loc[0,'County'])&(sites_df['Site Code']==xls_df.loc[0,'Site'])]
...
#                print (delta)
                if delta>gap:
                    print(r'At {} {}) the parameter {} ({}) has a gap '\
                         r'greater than 14 days between {} and {}. Null '\
                         r'hourly data between these dates. Data are contained '\
                          r'in the file {}'.format(site_name,station, param, param_dict.get(param), date1, date2, filename))
                    print('\n')
                    gap_count+=1
...
for state, county, site, parameter, date, file in zip(pzs_fail_df['State'], 
    ...
    site_name='{} ({}, '.format(site_name['SITE NAME'].values[0].strip(),site_name['Site Symbol'].values[0])
    print('{} ({}) at {}{}-{}-{}) failed PZS on {} (Data from file {})'.format(parameter,param_dict.get(parameter), site_name,state, county, site, date, file))
